...
class DayScreen extends React.Component{
    ...
    componentDidMount(){
        let dateStr = `${this.props.timeCurrent.format("DD-MM-YYYY")}`
        GetQuote(dateStr,(data)=>{
            this.setState({dataQuote: data.data})
        })
    }
    getImage = () =>{
        let date = this.props.timeCurrent.format('DD/MM/YYYY');
        let hashCode = s => s.split('').reduce((a,b)=>{a=((a<<5)-a)+b.charCodeAt(0);return a&a},0)
        let idx = Math.abs( hashCode(date) %(asset.imgBackGrounds.length -1))
        return asset.imgBackGrounds[idx ]; 
    }
    render() {
        const {dataQuote} = this.state;
        return (
            <ImageBackground source={asset.weather.lightsky} imageStyle={{opacity:0.2}} style={styles.container}>
                ...
                />
                <Quote
                    contentQuote={dataQuote.tuViHangNgay_content}
                    authorQuote={dataQuote.author}
                />
                <Footer
                    datetime = {this.props.timeCurrent}
                />
...
const styles = StyleSheet.create({
    ...
    weekday:{
        ...
        height:'20%',
        backgroundColor:'rgba(55,132,113,0.9)',
        fontSize:18,
        color:'white',
        textAlign:'center',
    ...
    detailDate:{
        backgroundColor:'rgba(32,141,133,0.8)',
        height:'80%',
