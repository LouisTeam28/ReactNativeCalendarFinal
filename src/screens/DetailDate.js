...
import { GetDayEvent, GetListNote, GetTuViHangNgay } from '../api';
...
class DetailDate extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            isCheckDayEvent: false,
        }
    }

    componentDidMount() {
        this.GetListNote();
        this.getTuViHangNgay();
        this.getDayEvent();
    }
    GetListNote = () => {
    ...
    getTuViHangNgay = () => {
        let dateStr = `${this.time.day}-${this.time.month}-${this.time.year}`

        GetTuViHangNgay(dateStr, (data) => {
            this.setState({ dataTuViHangNgay: data.data })
        })

    }
    getDayEvent = () => {
        let dateStr = `0${this.time.day}-0${this.time.month}-${this.time.year}`
        console.log("dateStr", dateStr)

        GetDayEvent(dateStr, (data) => {
            console.log("data.error.code", typeof (data.error.code))
            if (data.error.code == 200) {
                this.setState({ dataDayEvent: data.data[0], isCheckDayEvent: true })
                console.log("dataDayEvent", data.data[0])
            }
            if (data.error.code == 201) {
                // this.setState({isCheckDayEvent:false})
            }
        })
    ...
    render() {
        const { dataNote, dataTuViHangNgay, dataDayEvent, isCheckDayEvent } = this.state;
        ...
        return (
            <View style={styles.bigContainer}>
                ...
                >
                    ...
                    <Text style={styles.content}>{dataTuViHangNgay.tuViHangNgay_content}</Text>

                    <View style={styles.drawLine} />
                    <View>
                        <Text style={styles.title}> Các sự kiện : </Text>
                {isCheckDayEvent ? <Text style={styles.content}>{dataDayEvent.dayEvent_content}</Text> 
                                : <Text style={styles.contentNO}>Không có sự kiện !</Text> }
                </View>
